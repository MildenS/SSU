//5 сортировка BBall'ов 
//пришлось метод getColor() сделать константным для работы с BowlingComp(...)
#include <iostream>
...
bool BowlingComp(const BBall& b1, const BBall& b2);
...
{
    for (int i = 1; i < n; i++)
        for (int j = i; j > 0 && BowlingComp(arr[j], arr[j - 1]); j--)
            swap(arr[j - 1], arr[j]);
}
